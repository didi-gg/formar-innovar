...
class EdukreaLoginProcessor:
    """
    Procesa datos de login de Moodle para generar estadísticas de acceso con la base de datos de Edukrea.
        ...
        try:
            sql = f"""
                ...
                INNER JOIN '{students_enrollment}' AS estudiantes
                    ON logs.userid = estudiantes.edukrea_user_id
    ...
    def process_moodle_logins(self):
        # --- Paso 1: Cargar datos de Moodle Edukrea
        data = self._load_moodle_data(
        ...
        data["timecreated"] = pd.to_datetime(data["timecreated"], unit="s").dt.tz_localize("UTC")
        data["fecha_local"] = data["timecreated"].dt.tz_convert("America/Bogota")
        data["moodle_user_id"] = data["moodle_user_id"].astype(str)

        # --- Paso 3: Asignar periodo
        data["periodo"] = data["fecha_local"].apply(self.period_utils.assign_period)

        # --- Paso 4: Marcar logins en vacaciones ---
        data["en_vacaciones"] = data["fecha_local"].apply(self.period_utils.is_vacation)

        # --- Paso 5: Día de la semana y jornada ---
        data["hora"] = data["fecha_local"].dt.hour
        data["dia"] = data["fecha_local"].dt.day_name().str.lower()
        data["jornada"] = data["hora"].apply(self.period_utils.classify_daytime)

        # --- Paso 6: Calcular inactividad ---
        # Usar solo logins fuera de vacaciones para calcular `delta`
        resultado_no_vacaciones = data[~data["en_vacaciones"]].copy()
        ...
        summary = data.groupby(["moodle_user_id", "year", "periodo"]).agg(count_login=("fecha_local", "count")).reset_index()
        ...
        conteo_jornada = data.pivot_table(
        ...
        output_path = "data/interim/moodle/edukrea_logins.csv"
...
if __name__ == "__main__":
    processor = EdukreaLoginProcessor()
