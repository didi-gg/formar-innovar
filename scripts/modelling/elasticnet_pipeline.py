"""
Pipeline independiente para ElasticNet con tuning de hiperparámetros y análisis completo.
"""
...
class ElasticNetPipeline:
    """
    Pipeline independiente para ElasticNet con análisis completo y tuning de hiperparámetros.
    
    Características:
    - Encoding categórico automático
    - Imputación con mediana
    - Escalado estándar
    - Tuning de hiperparámetros (alpha, l1_ratio)
    ...
    def __init__(self, random_state: int = 42):
        """
        Inicializa el pipeline de ElasticNet.
    ...
    def _create_pipeline(self) -> Pipeline:
        """
        Crea el pipeline completo de ElasticNet.
        ...
        # 4. Modelo ElasticNet
        steps.append(('regressor', ElasticNet(random_state=self.random_state)))
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> 'ElasticNetPipeline':
        """
        Entrena el modelo ElasticNet con tuning de hiperparámetros.
        ...
        Returns:
            self
        """
        print(f"=== Entrenando ElasticNet con Tuning ===")
    ...
    def _cross_validate_with_best_params(self, X: pd.DataFrame, y: pd.Series):
        ...
        pipeline_with_best_params = self._create_pipeline()
        ...
        for param, value in self.best_params.items():
            ...
            if len(param_parts) == 2:
                ...
                pipeline_with_best_params.named_steps[step_name].set_params(**{param_name: value})
    ...
    def _plot_learning_curves(self):
        ...
        plt.ylabel('RMSE')
        plt.title('Curvas de Aprendizaje - ElasticNet')
        plt.legend()
    ...
    def get_feature_importance(self) -> Optional[pd.DataFrame]:
        """
        Obtiene la importancia de las características (coeficientes).
        
        Returns:
            DataFrame con coeficientes del modelo
        """
        if self.pipeline is None:
            return None
